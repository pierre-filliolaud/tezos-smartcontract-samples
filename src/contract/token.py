...
class TOKEN(sp.Contract):
    def __init__(self, owner, admin, manager, octo, covir):
        self.init(factor=10**6, transferStatus=False, saleStatus=False, balances=sp.big_map(), owner=owner, administrator=admin, saleManager=manager, octopus=octo, covir=covir, circulatingSupply=0, saleLimit=200000000*10**6, supplyLimit=400000000*10**6, soldToken=0, tokenSymbol="TOKEN", tokenName="Covir", ratio=1)
    ...
    def dispatchRoyalties(self, params):
        ...
        muTOKENtez = sp.as_nat(self.data.circulatingSupply)*rounded // params.amount + 1
        sp.for address in params.addresses:
            sp.if (self.data.balances.contains(address) & (sp.as_nat(self.data.balances[address].balance)*rounded > muTOKENtez)):
                sendMuTez = sp.as_nat(self.data.balances[address].balance)*rounded // muTOKENtez
...
if "templates" not in __name__:
    ...
    def test():
        ...
        c1 = TOKEN(owner, admin1, manager1, octopus, covir)
        scenario += c1
        ...
        scenario += c1.tokenDrop(addresses=[jack], amount=100 * factor).run(sender=jack, valid=False)
        ...
        scenario += c1.tokenDrop(addresses=[alice], amount=10 * factor).run(sender=admin1, valid=False)
        
        scenario.h3("Admin2 aidrop tokens to a list of one wallet: Alice")
        scenario += c1.tokenDrop(addresses=[alice], amount=10 * factor).run(sender=admin2)
        ...
        scenario += c1.tokenDrop(addresses=[alice, bob, jack], amount=10 * factor).run(sender=admin2)
        
        scenario.h3("Admin2 aidrop too many tokens (over supply limit)")
        scenario += c1.tokenDrop(addresses=[alice, bob, jack], amount=200000000 * factor).run(sender=admin2, valid=False)
        ...
        scenario.h3("Jack tries to send royalties to TOKEN holders")
        scenario += c1.dispatchRoyalties(addresses=[jack], amount= 100000000).run(sender=jack, amount=sp.tez(100), valid=False)
        
        # test not available on smartpy
        # scenario.h3("Admin2 tries to send royalties to TOKEN holders but do not have enough XTZ")
        ...
        scenario += c1.tokenDrop(addresses=[johndoe7], amount=200 * factor).run(sender=admin2)
        ...
        scenario.h3("Manager2 sales 1000 TOKEN to Johndoe11")
